...
protected:
	...
	std::weak_ptr<class ParticleSystem> thruster;

private:
	sf::Vector2f AvoidObstacles();
	std::weak_ptr<class GroupAttackingPolicy> groupAttack_;
	std::weak_ptr<class ShootingComponent> shootingComponent_;
	...
	std::weak_ptr<class GameEntity> target_;
	std::vector<std::weak_ptr<class GameEntity>> obstacles_;
