...
void ParticleSystem::EmitterUpdate(const float deltaTime)
{
	if (ShouldEmit())
	{
		...
		if (spawnElapsed_ > currentSpawnInterval_)
		{
			...
			{
				UpdateSpawnInterval(Maths::GetRandFloat(emitterRateMin_, emitterRate_));
			}
...
void ParticleSystem::BurstsUpdate()
{
	...
	for (auto& burst : bursts_)
	{
		...
		if (timeElapsed_ > totalDuration_ * burst.durationValue)
		{
			...
			{
				amt = (int)Maths::GetRandFloat((float)burst.countMin, (float)burst.count);
			}
			for (int i = 0; i < amt; i++)
			{
				SpawnParticle();
...
void ParticleSystem::CheckForExpiredLifetimes()
{
	for (int i = 0; i < activeParticles_.size(); ++i)
	{
		...
		//if waiting to die
		particle->SetPosition(Maths::ZeroVector());
		particle->SetScale(Maths::ZeroVector());
...
void ParticleSystem::InitialiseParticle(const std::weak_ptr<Particle>& particle)
{
	if (std::shared_ptr<Particle> p = particle.lock())
	{
		...
		{
			p->SetLifeTime(Maths::GetRandFloat(particleLifetime_, particleLifetimeMax_));
		}
		else
		{
			...
			case EmissionVelocityMode::Outward:
				{
					...
					direction = Maths::Normalize(pPos - mainPos);
				}
				break;
		...
		float angle = Maths::RadsToDegs(atan2f(direction.y, direction.x)) + 90.f;
		...
		{
			velocity = direction * Maths::GetRandFloat(particleSpeedMin_, particleSpeed_);
		}
		else
		{
		...
		{
			p->SetRenderColor(Maths::LerpColor(particleRandomColorRangeFrom_,
												 particleRandomColorRangeTo_,
												 Maths::GetRandFloat(0.f, 1.f)));
		}
		else
		{
