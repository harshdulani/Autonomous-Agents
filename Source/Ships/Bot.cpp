...
void Bot::Update(const float deltaTime)
{
	...
	auto angle = atan2f(lookVector.y, lookVector.x) - Maths::PI * 0.5f;
	angle = Maths::WrapModulo(angle, Maths::TWO_PI);
	SetRotation(Maths::LerpUnclamped(GetRotation(), angle, 3.5f * deltaTime));
}
...
void Bot::OnCollision(std::weak_ptr<GameEntity> other)
{
	...
	{
		if (boid && Maths::DotProduct(that->GetForwardVector(), GetForwardVector()) > fov_)
		{
			TryAddToList<Bot>(boid, flock_);
		}

		if (Maths::GetVectorMagnitude(GetPosition() - that->GetPosition()) < GetSeparationRadius())
		{
...
void Bot::UpdateVelocity(const float deltaTime)
{
	...
	newVelocity = Maths::LimitVector(newVelocity, maxSpeed * deltaTime);
...
sf::Vector2f Bot::SeekLocation(const sf::Vector2f& location) const
{
	...
	sf::Vector2f desired = Maths::Normalize(direction) * maxSpeed;

	// Steering force
	sf::Vector2f steerForce = desired - velocity;
	steerForce = Maths::LimitVector(steerForce, maxSteerForce);
...
sf::Vector2f Bot::Separation()
{
	...
	for (int i = 0; i < separaters_.size(); i++)
	{
		...
		if (auto other = separaters_[i].lock())
		{
			...
			float distance = Maths::GetVectorMagnitude(direction);
			...
			float factor = 1.f - (Maths::InverseLerpUnclamped(0.f, GetSeparationRadius(), distance));
			
			sum += Maths::Normalize(direction) * factor;
			count++;
		}
	}
	if (count > 0)
	{
		...
		sum = Maths::Normalize(sum) * maxSpeed;
	...
	for (int i = 0; i < flock_.size(); i++)
	{
		...
		if (auto other = flock_[i].lock())
		{
			...
			if (other->IsPendingKill() ||
				...
				|| Maths::DotProduct(other->GetForwardVector(), GetForwardVector()) < fov_
				)
			{
				// remove refs to dead or faraway friends
			...
			// the farther away someone is from us, lesser the force to be applied
			float factor = 1.f - (Maths::InverseLerpClamped(0.f, GetSeparationRadius(), distance));
			sum += Maths::Normalize(direction) * factor;
			count++;
		}
	}
...
sf::Vector2f Bot::Cohesion()
{
	...
	for (int i = 0; i < flock_.size(); i++)
	{
		...
		if (auto other = flock_[i].lock())
		{
			float distance = Maths::GetVectorMagnitude(GetPosition() - other->GetPosition());
