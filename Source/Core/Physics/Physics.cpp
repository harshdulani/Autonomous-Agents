...
std::weak_ptr<GameEntity> Physics::Raycast(const sf::Vector2f& start, const sf::Vector2f& end, const GameEntity* self)
{
	...
	int i = static_cast<int>(ceilf(Maths::GetVectorMagnitude(start - end)));
	int j = 0;
	std::vector<sf::Vector2f> traversed;
	while (j++ < i)
	{
		float x = Maths::LerpClamped(startX, endX, (float)j / (float)i);
		float y = Maths::LerpClamped(startY, endY, (float)j / (float)i);

		int cellX = Maths::InverseLerpUnclamped(0, grid->gridWidth_, x);
		int cellY = Maths::InverseLerpUnclamped(0, grid->gridHeight_, y);
		...
		if (static_cast<int>(traversed.size()) > 0)
		{
			...
			if (Maths::IsAlmostEqualTo(nu, item))
			{
				continue;
			}
		}
...
void Physics::RaycastAll(const sf::Vector2f& start, const sf::Vector2f& end, const GameEntity* self, std::vector<std::weak_ptr<GameEntity>>& inVector)
{
	...
	int i = (int)ceilf(Maths::GetVectorMagnitude(start - end));
	int j = 0;
	std::vector<sf::Vector2f> traversed;
	while (j++ < i)
	{
		...
		float cellX = Maths::InverseLerpUnclamped(0.f, halfWidth, x);
		float cellY = Maths::InverseLerpUnclamped(0.f, halfHeight, y);
		...
		while (objectsMask)
		{
			...
			if (auto other = collidingEntity.lock())
			{
				if (other->HasColliders() && !other->IsPendingKill())
				{
					...
					if (it == inVector.end())
					{
						if (Maths::GetVectorMagnitude(other->GetPosition() - position)
							< radius)
						{
