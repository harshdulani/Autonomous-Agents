...
{
	if (!font_.loadFromFile(System::GetAssetPath("RecursiveSansLnrSt-Med.ttf")))
	{
		// error...
	}
								   ...
								   int& x1, int& y1, int& x2, int& y2) const
{
	...
	x1 = Maths::ClampValue(x1, 0, gridCellCountX_ - 1);
	x2 = Maths::ClampValue(x2, 0, gridCellCountX_ - 1);
	y1 = Maths::ClampValue(y1, 0, gridCellCountY_ - 1);
	y2 = Maths::ClampValue(y2, 0, gridCellCountY_ - 1);
}
